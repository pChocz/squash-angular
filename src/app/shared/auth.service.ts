...
import { SeasonDto } from '../all-leagues-view/model/season-dto.model';
...
export class AuthService {
  ...
  public async hasRoleForLeagueForSeason(seasonUuid: string, role: string): Promise<boolean> {
    ...
    const season = await this.http.get<Season>(environment.apiUrl + 'seasons/' + seasonUuid)
    .pipe(
  ...
  public async hasRoleForLeagueForRound(roundUuid: string, role: string): Promise<boolean> {
    ...
    const leagueUuid = await this.http.get<string>(environment.apiUrl + 'rounds/' + roundUuid + "/leagueUuid")
    .pipe(
      map(result => plainToClass(String, result)))
    .toPromise();
